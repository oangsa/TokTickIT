...
- Exact focused Issue 2 gate: 12 files, 24 tests passed with `@issue-2` when local server binding was allowed.
...
- The real PostgreSQL Issue 2 gate passed: 5 suites, 5 tests. It first exposed Prisma's inability to deserialize the `void` result from `pg_advisory_xact_lock`; the rate-limit lock now returns a scalar sentinel through a CTE, and the rerun passed.
- The current PG gate fails closed before tests run when the required explicit target overrides/baselines are absent.
- No secrets or plaintext refresh/password credentials logged by implementation.
