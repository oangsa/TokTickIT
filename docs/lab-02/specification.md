...
- **BR-15** Missing, malformed, non-positive, unknown, inactive, or logically deleted `X-Requester-Id` context produces `400 Bad Request` with the protocol-specific code `REQUESTER_CONTEXT_INVALID` using the centralized safe error envelope. The six conditions are indistinguishable from each other, and the message never discloses which one occurred. `401` and `403` are not used for this behavior.
- **BR-15a** `REQUESTER_CONTEXT_INVALID` is the only response the frontend may treat as "the stored Requester context is invalid and must be cleared". Ordinary application `400` responses, including `BAD_REQUEST` and `VALIDATION_ERROR`, must never clear the Requester context.
...
- **BR-17** When Requester context changes or is invalidated, the shared requester layer clears the stored Requester context, shared requester provider/context state, and shared requester-scoped cache infrastructure, cancels in-flight shared requester requests, and rejects responses that arrive from the previous Requester. It exposes a registration/clearing seam that requester-scoped features use to clear their own state through the same switch.
- **BR-17a** Each requester-scoped feature clears its own feature-specific state through that shared seam: Create Ticket draft, Pending Attachment preparation, and idempotency/recovery state; My Tickets rows, query/filter/sort/page/count state, and list requests/stale list responses; Ticket Detail state and Detail requests; and Attachment lifecycle/request state. Whatever the owner, Requester A data must never render under Requester B.
...
- **BR-46** Maximum attachment size is exactly `5,000,000` bytes per file (`MAX_ATTACHMENT_BYTES`, decimal 5 MB). The same constant is used by frontend validation, multipart parsing, backend validation, PostgreSQL constraints, documentation, and tests.
...
- **BR-63** Files larger than `5,000,000` bytes return `413 Content Too Large`.
...
- **BR-91** Every interactive icon-only control provides both an accessible programmatic name and a visible tooltip or equivalent hover/focus label that communicates the action. This applies to navigation, Attachment, close/dismiss, pagination, filter/search auxiliary, mobile-sidebar, modal, and other icon-only controls.
...
Accessibility baseline includes semantic labels, keyboard-accessible controls, visible focus states, field-associated validation messages, both accessible programmatic names and visible tooltips/hover-focus labels for every icon-only action, modal focus management, non-color-only status communication, and live/status announcements for asynchronous upload/submission states where appropriate.
...
The header behaves operationally like the current Lab 2 Requester context/session but is not authentication, an authorization token, or a security credential. Missing, malformed, non-positive, unknown, inactive, or deleted Requester context returns `400` with the protocol-specific code `REQUESTER_CONTEXT_INVALID`; a requester-owned resource outside the valid current Requester's scope returns the same centralized `404` as an unavailable resource.
...
A specific code is allowed where the frontend must distinguish protocol behavior. Lab 2 uses `IDEMPOTENCY_CONFLICT` and `REQUESTER_CONTEXT_INVALID` (`400`, the single machine-readable invalid-Requester-context signal).

Attachment status mapping includes unsupported extension `415`, file larger than `5,000,000` bytes `413`, active-limit or non-bindable conflict `409`, removed preview/download `410`, repeat removal of an already Removed Attachment `404`, and exhausted serialization/deadlock retries `500 INTERNAL_SERVER_ERROR`.
...
- **AC-13** Given a permitted non-empty file of `4,999,999` or `5,000,000` bytes is selected before Ticket submission, when `POST /api/attachments` succeeds, then PostgreSQL stores one owned Pending Attachment and the frontend receives a full `AttachmentDTO` with its opaque storage key and `ticketPublicId: null`; every permitted non-empty file no larger than `5,000,000` bytes is accepted by the same rule.
...
- **AC-15** Given a file of `5,000,001` bytes or larger, when upload is attempted, then the API returns `413` and does not create a usable Attachment.
...
- **AC-38** Given keyboard-only interaction, when the user navigates Requester selection, forms, filters, dialogs, pagination, and Attachment controls, then all required actions remain operable with visible focus and associated accessible labels/errors; every interactive icon-only control also has both an accessible programmatic name and a visible tooltip or equivalent hover/focus label.
...
- **AC-41** Given no Requester is selected, when `GET /api/requesters` is called, then it can bootstrap the selector without `X-Requester-Id`; given any other Lab 2 endpoint, when Requester context is missing, malformed, non-positive, unknown, inactive, or deleted, then the API returns safe `400 REQUESTER_CONTEXT_INVALID` with the same generic message in every case.
...
- **AC-46** Given no valid Requester context is stored, when a requester-specific route is opened, then the UI redirects to `/requesters` before requester data renders; given a stored Requester context later produces `400 REQUESTER_CONTEXT_INVALID`, then the frontend clears requester context/state and redirects to `/requesters`; given an ordinary application `400` without that code, then the stored Requester is left untouched.
...
- **AC-56** Given Attachment persistence, PostgreSQL rejects invalid Pending/Active/Removed combinations, untrimmed/invalid Removed reasons, names outside 1-255 UTF-8 bytes, binary sizes outside 1-5,000,000 bytes, or `size_bytes` unequal to `octet_length(data)`; the authoritative size invariant is `size_bytes > 0 AND size_bytes <= 5000000 AND size_bytes = octet_length(data)`.
...
- **AC-60** Given requester-scoped responses and representative errors, `Cache-Control: no-store` is present and `Vary` merges `Origin, X-Requester-Id` without overwriting the value CORS already applied; given the bootstrap `GET /api/requesters`, `Cache-Control: no-store` is present and the CORS `Vary: Origin` is preserved, but `X-Requester-Id` is not added because the response is not requester-dependent.
...
- UI tests cover sidebar/mobile navigation, Requester guards, visible generated Ticket Number/Date and Requester context, contract-significant editable/read-only/invalid/button/badge/Attachment presentation states, required controls, validation, busy states, mandatory accessible programmatic names plus visible tooltip/hover-focus labels for representative icon-only controls, list behavior, Pending pre-upload states and Retry, unresolved-file submit blocking, discard cleanup, `4xx` retention, `5xx` compensation/recovery, same-key ambiguous replay, Active-after-create behavior, batch removal, preview, and global errors.
