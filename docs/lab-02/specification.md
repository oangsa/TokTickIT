...
- An explicit idempotent maintenance CLI for expired Pending Attachments and logically expired `COMPLETED` Idempotency Records; production scheduling remains external to Lab 2. Stale `PROCESSING` recovery is request-driven through the approved five-minute claim lease and is not maintenance cleanup.
...
- **BR-19** The same Requester using the same Idempotency Key with the same canonical logical payload shall not create a second Ticket. A new operation first owns the unique `(requesterId, key)` by inserting a `PROCESSING` claim with `processingStartedAt = now` before mutable business validation or Ticket/Attachment mutation. `PROCESSING_STALE_AFTER` is exactly five minutes. An identical request against a fresh claim (`now < processingStartedAt + 5 minutes`) waits within the normal request timeout; against a stale claim (`now >= processingStartedAt + 5 minutes`) it may atomically reclaim the existing claim and reset `processingStartedAt = now`. Each owner retains the exact `processingStartedAt` value representing its lease and must pass the approved `IDEMPOTENCY-FENCING-A` ownership check before any resource mutation. The first successful fenced owner transitions that claim to `COMPLETED` and returns `201 Created`; a completed replay returns `200 OK` for the same Ticket creation identity with a freshly reconstructed current `TicketDTO`.
...
- **BR-21** Ticket-create canonicalization uses stable property ordering, validates UUID syntax, rejects duplicate `attachmentIds`, normalizes identifiers, and sorts them deterministically. Omitted and empty `attachmentIds` both mean no initial Attachments. `[A, B]` and `[B, A]` are equal; `[A, B]` and `[A, C]` are different. The canonical logical request is encoded as UTF-8 bytes, hashed with SHA-256, and stored as exactly 64 lowercase hexadecimal characters in `request_hash VARCHAR(128)`.
...
- **BR-76** Direct upload to an existing Ticket enforces the active-five limit inside a Prisma transaction at PostgreSQL `Serializable` isolation. Serialization/deadlock conflicts receive at most three bounded retries with small randomized backoff; exhaustion returns `503 SERVICE_UNAVAILABLE` and may include `Retry-After: 1`.
...
| `isActive Boolean` | `is_active BOOLEAN` | `NOT NULL` | `@default(true)` / `DEFAULT true` | — |
| `deleted Boolean` | `deleted BOOLEAN` | `NOT NULL` | `@default(false)` / `DEFAULT false` | — |
| `createdBy`, `createdAt`, `updatedBy`, `updatedAt` | Shared audit columns | `NOT NULL` | Shared audit behavior; existing `createdAt` values preserved | Section 7.2.1 |
...
| `processingStartedAt DateTime` | `processing_started_at TIMESTAMPTZ` | `NOT NULL` | Backend sets server/database time when the claim is inserted and resets it when a stale same-hash claim is reclaimed; no database default | Five-minute Processing lease; retained unchanged when the row becomes Completed |
...
The Lab 1 Category table is migrated forward in place rather than dropped/recreated. The migration preserves existing IDs, names, and original `createdAt` values; renames table/columns to the Lab 2 mapped snake_case convention; adds/backfills new audit/lifecycle columns before enforcing `NOT NULL`; and uses `seed` as the backfill actor without rewriting original creation timestamps. Verification covers both a fresh database and a populated Lab 1 database upgraded through the committed migration.
...
`attachmentIds` is optional; omitted and empty mean no initial Attachments. It contains at most five valid UUID/storage keys, rejects duplicates, and is normalized/sorted as an unordered logical set. Requester ID, public ID, Ticket Number, Current Status, timestamps, deletion flag, and audit fields are backend-derived/generated.
...
- `SERVICE_UNAVAILABLE` - 503 after bounded serialization/deadlock retry exhaustion.

A specific code is allowed where the frontend must distinguish protocol behavior, such as `IDEMPOTENCY_CONFLICT`.

Attachment status mapping includes unsupported extension `415`, file larger than `5,242,880` bytes `413`, active-limit or non-bindable conflict `409`, removed preview/download `410`, repeat removal of an already Removed Attachment `404`, and exhausted serialization/deadlock retries `503`.
...
- **AC-47** Given an existing Ticket has four Active Attachments, when two direct uploads race, then serializable max-five enforcement produces exactly one `201`, one `409`, and five Active rows. Serialization/deadlock failures receive at most three retries; exhaustion returns `503 SERVICE_UNAVAILABLE` and may include `Retry-After: 1`.
...
- **AC-49** Given a populated Lab 1 Category table, when the Lab 2 migration is applied, then existing Category IDs, names, and original creation timestamps survive the in-place snake_case/audit/lifecycle migration; a fresh database remains reproducible from migrations and seed.
...
- **AC-65** Given canonical Ticket-create input, when the idempotency hash is computed and claims transition, then UTF-8 SHA-256 produces deterministic 64-character lowercase hexadecimal output, sorted normalized Attachment IDs make `[A,B]` equal `[B,A]` while different/duplicate sets behave as specified, only valid `PROCESSING` and `COMPLETED` database states persist, and a valid fenced claim transitions `PROCESSING -> COMPLETED` atomically with final mutable validation, Ticket creation, and binding while its claim-row lock remains held. Fresh same-hash `PROCESSING` waits, stale begins at exactly five minutes, and two concurrent stale same-hash retries produce exactly one conditional reclaim owner with `processingStartedAt` reset, exactly one Ticket/binding result, and one normal waiter/replay; different-hash requests conflict whether the claim is fresh or stale. If an original slow owner resumes after reclaim with its old `processingStartedAt`, its fencing check fails before mutation and only the reclaim owner may create, bind, and complete.
...
23. **Handout API examples:** API snippets and payload examples in the Lab handout are partial and illustrative unless the handout explicitly states that an example is a fixed requirement. They inform the Lab 2 design but are not complete wire contracts. The frozen routes, DTOs, headers, query semantics, status codes, and errors in `docs/lab-02/api-spec.md` are the authoritative Lab 2 implementation contract.
