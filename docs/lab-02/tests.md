...
| Test ID | Type | Requirement / AC | What It Tests | Expected Result | Automated Test File | Final |
| --- | --- | --- | --- | --- | --- | --- |
| API-01 | API | AC-05, AC-41, AC-46 | Requester-context middleware and bootstrap exception. | `GET /api/requesters` works without `X-Requester-Id`; every other Lab 2 endpoint rejects missing, malformed, non-positive, unknown, inactive, or deleted context with safe 400 behavior. | tests/lab-02/requester-context.api.test.ts | Passed |
...
| API-03 | API | BR-07, BR-71–73 | Retrieve active Categories. | 200 raw array; only active/non-deleted categories; valid requester header required. | tests/lab-02/reference-data.api.test.ts | Not Run — the `CategoryDTO` conversion is owned by Issue #21; Issue #20 only put the existing route behind the requester-context guard and did not change its response body. |
...
| API-72 | API | AC-58 | JSON parser size/error classification. | A body at the 131,072-byte parser boundary reaches normal parse/downstream handling rather than size rejection; larger returns 413; malformed JSON within limit returns 400 BAD_REQUEST; valid JSON with invalid fields returns 400 VALIDATION_ERROR. | tests/lab-02/transport-hardening.api.test.ts | Partial — the 131,072-byte boundary, the 413, the malformed-JSON 400, and the header `VALIDATION_ERROR` are covered by `transport-hardening.api.test.ts`; the request-body field `VALIDATION_ERROR` needs a field-validating endpoint and is owned by Issue #21. |
...
| API-74 | API | AC-60 | Cache and variation headers. | Bootstrap, requester-scoped JSON/binary, and representative error responses use no-store; requester-dependent responses merge Origin and X-Requester-Id into Vary without clobbering CORS values. | tests/lab-02/transport-hardening.api.test.ts | Passed |
...
| UI-04 | UI | AC-01, AC-05, AC-46 | Requester route guard and invalid-context handling. | No valid stored context redirects requester routes to `/requesters` before requester data renders; defined invalid-context 400 clears context/state and redirects. | tests/lab-02/ApplicationShell.test.tsx | Passed — the route-guard half is covered by `ApplicationShell.test.tsx` and the invalid-context `400` half is covered by the `useRequesterApi` cases added in this Issue. |
