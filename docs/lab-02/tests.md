...
6. **The Ticket row needed a semantic keyboard target.** The earlier fix made
   the `<tr>` itself focusable with an `aria-label` and hand-rolled Enter/Space
   handling, so assistive technology announced a row rather than an activatable
   control. The Ticket Number cell now owns one native `<Link>`: it keeps the
   visible Ticket Number in the accessible name, supports native keyboard and
   new-tab behavior, and preserves the list query in its URL. The row's
   `onClick` remains a pointer convenience for the other cells, while
   `.tt-row:has(.tt-row-link:focus-visible)` draws the keyboard focus ring.
   ui-spec Section 16.2 permits this explicitly — "activatable with Enter/Space
   where the chosen implementation pattern supports it".
...
| UI-08 | UI | FR-12 | Create Ticket busy submission and recovery action. | Delayed response disables the current primary action with a spinner while preserving its text (`Submit Ticket` or `Retry Again`); duplicate click prevented. Ambiguous failure changes the same primary action to `Retry Again` and renders no separate recovery button. A context-invalidating `400` is treated as a confirmed non-ambiguous failure, so it discards the stored context and leaves no recovery record. | tests/lab-02/CreateTicket.test.tsx | Passed |
...
| UI-14 | UI | AC-45 | Create Ticket Cancel/discard and guarded shell navigation. | Untouched empty draft cancels directly; dirty, known Pending, or recovery state requires confirmation; Cancel, sidebar My Tickets, and Change Requester share the same discard flow. Confirm sends best-effort Pending cleanup, clears fields/files/recovery, and completes the requested navigation. | tests/lab-02/CreateTicket.test.tsx | Passed — direct cancel, sidebar navigation, Change Requester, confirmation, Keep editing, and confirmed discard clearing the draft/recovery are covered in `CreateTicket.test.tsx`; the best-effort Pending cleanup call, with an empty reason per item, is covered in `AttachmentSection.test.tsx`. |
...
| UI-16 | UI | AC-34 | My Tickets empty dataset vs no-results states. | Shared EmptyState shows correct distinct copy/actions; no-results uses the toolbar's single Clear Filters action rather than duplicating it in the empty state. | tests/lab-02/MyTickets.test.tsx | Passed |
...
| UI-28 | UI | AC-19 | Batch Attachment selection. | Only Active Ticket Detail rows are selectable; one selected row uses its row Remove action, while Remove Selected appears only for two or more selected rows; Create Ticket transient/Pending states and Removed rows are not selectable for Active removal. | tests/lab-02/AttachmentSection.test.tsx | Passed — only Active Ticket Detail rows carry a checkbox, the selected count and single-row/batch action threshold behave correctly, and a Create Ticket Pending row offers neither. |
...
| UI-33 | UI | AC-51–53 | Ambiguous-create recovery persistence and expiry. | Recovery record stores requester/key time/original normalized payload only while ambiguous; reload restores an explicit `Retry Again` primary action without auto-submit or a second recovery button; success/failure/discard/switch/expiry clears it; current replay rendering includes later Attachment mutations. | tests/lab-02/CreateTicket.test.tsx | Passed — `CreateTicket.test.tsx` proves requester-scoped persistence, approved fields, no auto-submit, `Retry Again` same-key replay, 24-hour expiry, and terminal clearing; API replay tests prove current Attachment reconstruction; E2E-03 proves the persisted recovery record resumes after reload without duplicate creation or re-upload. |
