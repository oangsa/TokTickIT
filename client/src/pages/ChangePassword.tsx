...
function passwordSchema(restricted: boolean) {
  const password = z.string().refine((value) => Array.from(value).length >= AUTH_FORM_RULES.password.minLength, `Password must be at least ${AUTH_FORM_RULES.password.minLength} characters.`).refine((value) => Array.from(value).length <= AUTH_FORM_RULES.password.maxLength, `Password must be no more than ${AUTH_FORM_RULES.password.maxLength} characters.`).refine((value) => /[A-Z]/.test(value), "Password needs an uppercase letter.").refine((value) => /[a-z]/.test(value), "Password needs a lowercase letter.").refine((value) => /\p{Nd}/u.test(value), "Password needs a number.").refine((value) => /[\p{P}\p{S}]/u.test(value), "Password needs a symbol.");
...
function ChangePasswordForm({ restricted }: { restricted: boolean }) {
  ...
  return <main id="tt-main" tabIndex={-1} className="tt-bootstrap"><div className="tt-bootstrap__panel"><Card><PageHeader title="Change Password" subtitle={restricted ? "You must choose a new password before continuing." : "Update your password for this account."} /><p className="text-secondary">Use 8–128 characters with at least one uppercase letter, lowercase letter, number, and symbol. Spaces are allowed but do not count as symbols. The new password must differ from the current password.</p><CommonForm form={form} sections={sections} onSubmit={submit} showCancelButton={false} submitLabel="Change Password" submitting={submitting} ariaLabel="Change Password" /></Card></div></main>;
}
