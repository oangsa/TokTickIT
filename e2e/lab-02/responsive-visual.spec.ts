...
async function loadFixture(request: APIRequestContext, accessToken: string): Promise<Fixture> {
  const categories = await getJson<NamedRecord[]>(request, "/api/categories", accessToken);
  const relatedSystems = await getJson<NamedRecord[]>(request, "/api/related-systems", accessToken);
...
async function createTicketViaApi(
  ...
  if (attachmentName !== undefined) {
    const upload = await request.post(`${API_BASE_URL}/api/users/me/attachments`, {
      headers: { Authorization: `Bearer ${accessToken}` },
  ...
  const response = await request.post(`${API_BASE_URL}/api/users/me/tickets`, {
    headers: {
...
for (const viewport of VIEWPORTS) {
  ...
  test(`RESP-01/VIS-01 Create Ticket at ${label}`, async ({ page }) => {
    ...
    try {
      ...
      await page.getByLabel("Description *", { exact: true }).fill(`${summary} description for responsive verification.`);
      ...
      const systemBox = await page.getByLabel("Related System *", { exact: true }).boundingBox();
  ...
  test(`RESP-02/VIS-02 My Tickets at ${label}`, async ({ page, request }) => {
    ...
    try {
      const fixture = await loadFixture(request, session.accessToken);
      const marker = `Responsive List ${viewport.width} ${Date.now()}`;
      const ticket = await createTicketViaApi(request, fixture, session.accessToken, marker);
      ...
      if (viewport.width >= 992) {
        for (let index = 0; index < 12; index += 1) {
          await createTicketViaApi(
            ...
            fixture,
            session.accessToken,
            `Scrollable List ${viewport.width} ${index} ${Date.now()}`,
          );
        }
      }
      ...
      if (viewport.width < 992) {
        ...
        await expect(sidebar.getByRole("button", { name: "Logout", exact: true })).toBeVisible();
