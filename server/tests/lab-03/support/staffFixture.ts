...
export const actor = (user: { id: number; publicId: string; email: string; role: UserRole } = STAFF): TicketActor => ({ userId: user.id, userPublicId: user.publicId, email: user.email, role: user.role });
